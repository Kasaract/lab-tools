...
export function CellCountPage() {
    ...
    async (files: FileList | File[]) => {
      ...
      for (const archive of archiveFiles) {
        try {
          ...
          const extracted = res.images.map((img) =>
            b64toFile(img.data, img.name),
          );
  ...
  const analyze = useCallback(async () => {
    ...
    try {
      ...
      formData.append(
        "watershed_footprint",
        String(settings.watershedFootprint),
      );
  ...
  const exportAllImages = useCallback(async () => {
    ...
    for (const img of analyzedImages) {
      const baseName = img.name.replace(/\.[^.]+$/, "");
      zip.file(`${baseName}_analyzed.png`, img.result!.overlay, {
        base64: true,
      });
    }
    ...
    <div className="mx-auto max-w-7xl">
      <div className="flex gap-4">
        ...
        <div className="w-56 shrink-0 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3 self-start">
          <h3 className="text-xs font-semibold text-gray-700 dark:text-gray-200 mb-3">
            Detection Settings
          </h3>
          ...
          <div className="space-y-3">
            ...
            <SettingSlider
              label="Sensitivity"
              tooltip="Threshold multiplier for stain detection. Lower = detects fainter cells."
              value={settings.posThresh}
              min={0.1}
              max={1.5}
              step={0.05}
            ...
            <SettingSlider
              label="Min Area"
              tooltip="Minimum cell size in pixels. Smaller values detect tinier cells but may pick up noise."
              value={settings.posMinArea}
              min={5}
              max={200}
              step={5}
            ...
            <SettingSlider
              label="Cleanup"
              tooltip="Morphological disk radius. Higher values smooth more noise but may erode small cells."
              value={settings.posDiskRadius}
              min={1}
              max={5}
              step={1}
            ...
            <p className="text-[10px] font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide pt-1">
              Negative (Hematoxylin)
            </p>
            <SettingSlider
              label="Sensitivity"
              tooltip="Threshold multiplier for stain detection. Lower = detects fainter cells."
              value={settings.negThresh}
              min={0.1}
              max={1.5}
              step={0.05}
            ...
            <SettingSlider
              label="Watershed"
              tooltip="Controls how aggressively touching cells are split apart. Lower = more splitting."
              value={settings.watershedFootprint}
              min={3}
              max={20}
              step={1}
        ...
        <span className="text-[11px] text-gray-600 dark:text-gray-400 flex items-center gap-1">
          ...
          {tooltip && (
            <span className="relative group">
              <svg
                className="h-3 w-3 text-gray-400 dark:text-gray-500 cursor-help"
                fill="none"
                stroke="currentColor"
                strokeWidth={2}
                viewBox="0 0 24 24"
